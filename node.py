...
class Node:
    ...
    def draw_node(self, world):
        ...
        py.draw.circle(
            world.win, colorScheme[0], (self.x, self.y), NODE_RADIUS)
        py.draw.circle(
            world.win, colorScheme[1], (self.x, self.y), NODE_RADIUS - 2)

        # Draw labels
        if self.type != MIDDLE:
            text = NODE_FONT.render(self.label, 1, BLACK)
            world.win.blit(text, (self.x + (self.type-1) * ((text.get_width()
                                                             if not self.type else 0) + NODE_RADIUS + 5), self.y - text.get_height()/2))
    ...
    def getNodeColors(self, world):
        ...
        col = [[0, 0, 0], [0, 0, 0]]
        for i in range(3):
            col[0][i] = int(ratio * (self.color[1][i] -
                                     self.color[3][i]) + self.color[3][i])
            col[1][i] = int(ratio * (self.color[0][i] -
                                     self.color[2][i]) + self.color[2][i])
        return col
