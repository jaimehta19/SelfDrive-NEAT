...
class Car:
    ...
    def initImgs(self):
        img_names = ["yellow_car.png", "red_car.png",
                     "blu_car.png", "green_car.png"]
        # prendi a caso una di queste immagini
        name = img_names[floor(random()*len(img_names)) % len(img_names)]

        self.img = py.transform.rotate(py.transform.scale(py.image.load(
            os.path.join("img", name)).convert_alpha(), (120, 69)), -90)
        self.brake_img = py.transform.rotate(py.transform.scale(py.image.load(
            os.path.join("img", "brakes.png")).convert_alpha(), (120, 69)), -90)
    ...
    def getInputs(self, world, road):  # win serve per disegnare i sensori se DBG = True
        ...
        for v in [road.pointsLeft, road.pointsRight]:
            ...
            while v[i].y > self.y - SENSOR_DISTANCE:
                ...
                getDistance(world, self, sensors,
                            sensorsEquations, v[i], v[next_index])
                i = next_index
        ...
        if CAR_DBG:
            for k, s in enumerate(sensors):
                ...
                if s < SENSOR_DISTANCE:
                    py.draw.circle(world.win, RED, world.getScreenCoords(
                        self.x+dx, self.y+dy), 6)
    ...
    def move(self, road, t):
        ...
        # sottraggo perchè l'origine è in alto a sinistra
...
# restituisce le equazioni delle rette (in variabile y) della macchina in ordine [verticale, diagonale crescente, orizzontale, diagonale decrescente]
def getSensorEquations(self, world):
    ...
    for i in range(4):
        ...
        if CAR_DBG:  # disegna linee dei sensori
            py.draw.lines(world.win, GREEN, False, [world.getScreenCoords(
                self.x+dx, self.y+dy), world.getScreenCoords(self.x-dx, self.y-dy)], 2)
...
# dato il segmento (m,q) calcolo la distanza e la metto nel sensore corrispondente
def getDistance(world, car, sensors, sensorsEquations, p, q):
    ...
    for i, (a1, b1, c1) in enumerate(sensorsEquations):
        ...
        if a1 != a2 or b1 != b2:
            ...
            y = (a1*c2 - c1*a2)/d
            x = (c1*b2 - b1*c2)/d
            # se l'intersezione non sta tra a e b, vai alla prossima iterazione
        ...
        # angolo della retta del sensore (e del suo opposto)
        omega = car.rot + 45*i
        # angolo rispetto alla verticale (come car.rot)
