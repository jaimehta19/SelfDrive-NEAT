...
class Road:
    def __init__(self, world):
        ...
        # riempi i vettori pointsLeft e pointsRight
    ...
    def calcBorders(self, i):
        ...
        self.pointsLeft[i].y = center.y - y if not center.y - \
            y >= self.pointsLeft[prev_index].y else self.pointsLeft[prev_index].y
        self.pointsRight[i].x = center.x + x
        self.pointsRight[i].y = center.y + y if not center.y + \
            y >= self.pointsRight[prev_index].y else self.pointsRight[prev_index].y
    ...
    def createSegment(self, index):
        ...
        # invertiti perchè scify vuole le x crescenti (in questo caso le y)
        ...
        self.last_ctrl_point = getPoint(
            self.last_ctrl_point+1, self.num_ctrl_points)
    ...
    def draw(self, world):
        ...
        if(ROAD_DBG):
            ...
            for i in range(len(self.pointsLeft)):
                py.draw.circle(world.win, BLUE, world.getScreenCoords(
                    self.pointsLeft[i].x, self.pointsLeft[i].y), 2)
                py.draw.circle(world.win, BLUE, world.getScreenCoords(
                    self.pointsRight[i].x, self.pointsRight[i].y), 2)
